...
avl_array<T, A, W, P>::erase(typename avl_array<T, A, W, P>::iterator it) {
  return erase_it(it);
}

template <class T, class A, class W, class P>
inline typename avl_array<T, A, W, P>::reverse_iterator
...
template <class T, class A, class W, class P>
inline typename avl_array<T, A, W, P>::iterator
avl_array<T, A, W, P>::erase(typename avl_array<T, A, W, P>::iterator from,
...
avl_array<T, A, W, P>::extract_node(typename avl_array<T, A, W, P>::node_t *p) {
  node_t *q, *r, *w;
  size_type cl, cr;
  int side;
  ...
  AA_ASSERT_EXC(p->m_parent, invalid_op_with_end()); // Can't extract end()
...
template <class T, class A, class W, class P>
template <class IT>
inline IT avl_array<T, A, W, P>::erase_it(IT it) {
    ...
    typename avl_array<T, A, W, P>::size_type n) // # to erase
{
  ...
  while (n) { // Destruct removed nodes
    p = first;
    first = first->m_next;
    delete_node(p);
    n--;
