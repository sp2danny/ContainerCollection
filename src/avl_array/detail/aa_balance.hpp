...
void avl_array<T, A, W, P>::update_counters(
...
template <class T, class A, class W, class P>
// not inline
void avl_array<T, A, W, P>::update_counters_and_rebalance(
    typename avl_array<T, A, W, P>::node_t *p) {
  size_type i, j;
  int s;
  node_t *q, *r;
  ...
  {         // reached
        ...
        (s == L && i <= j))   // is enough...
    {
      ...
      */
      ...
      p->m_parent->m_children[1 - s] = p->m_children[s]; //   B       B
                                                         //  /  =>>  /^
      if (p->m_children[s])                              // A       Y A
        p->m_children[s]->m_parent = p->m_parent;
      //  C       C
      ...
      if (p->m_parent->m_children[L] == p->m_children[s]) //  C       C
