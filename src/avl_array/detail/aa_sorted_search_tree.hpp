...
template <class CMP>
inline bool avl_array<T, A, W, P>::binary_search(
    typename avl_array<T, A, W, P>::const_reference t,
    typename avl_array<T, A, W, P>::const_iterator &it, CMP cmp) const {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP, int, const_reference, const_reference>>();
#endif
...
inline bool avl_array<T, A, W, P>::binary_search(
    typename avl_array<T, A, W, P>::const_reference t,
    typename avl_array<T, A, W, P>::iterator &it, CMP cmp) {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP, int, const_reference, const_reference>>();
#endif
...
inline bool avl_array<T, A, W, P>::binary_search(
    typename avl_array<T, A, W, P>::const_reference t,
    typename avl_array<T, A, W, P>::const_reverse_iterator &it, CMP cmp) const {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP, int, const_reference, const_reference>>();
#endif
...
inline bool avl_array<T, A, W, P>::binary_search(
    typename avl_array<T, A, W, P>::const_reference t,
    typename avl_array<T, A, W, P>::reverse_iterator &it, CMP cmp) {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP, int, const_reference, const_reference>>();
#endif
...
inline bool avl_array<T, A, W, P>::binary_search(
    typename avl_array<T, A, W, P>::const_reference t,
    typename avl_array<T, A, W, P>::const_reverse_iterator &it) const {
  return binary_search(t, it, std::less<value_type>());
}

template <class T, class A, class W, class P>
inline bool avl_array<T, A, W, P>::binary_search(
    typename avl_array<T, A, W, P>::const_reference t) const {
  node_t *p;
...
template <class T, class A, class W, class P>
template <class CMP>
inline typename avl_array<T, A, W, P>::iterator
avl_array<T, A, W, P>::insert_sorted(
    typename avl_array<T, A, W, P>::const_reference t, bool allow_duplicates,
    CMP cmp) {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP, int, const_reference, const_reference>>();
...
avl_array<T, A, W, P>::insert_sorted(
    typename avl_array<T, A, W, P>::const_reference t, bool allow_duplicates) {
  return insert_sorted(t, allow_duplicates, std::less<value_type>());
}
...
void avl_array<T, A, W, P>::sort(CMP cmp) {
  ...
  while (next) {         // Take the elements of the list
...
template <class T, class A, class W, class P>
inline void avl_array<T, A, W, P>::sort() // Same, but with
{                                         // T::operator<
...
template <class T, class A, class W, class P>
inline void avl_array<T, A, W, P>::stable_sort() // Same, but with
{                                                // T::operator<
...
void avl_array<T, A, W, P>::merge(
    typename avl_array<T, A, W, P>::my_class &donor, CMP cmp) {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP, int, const_reference, const_reference>>();
#endif

  node_t *my_next, *donor_next, *next, *last, *first;
  size_type n;

  if (this == &donor || donor.size() == 0)
    return;

  if (size() == 0) {              // If *this is empty, just
...
template <class T, class A, class W, class P>
inline void
avl_array<T, A, W, P>::merge(typename avl_array<T, A, W, P>::my_class &donor) {
...
template <class T, class A, class W, class P>
inline void avl_array<T, A, W, P>::unique() {
...
template <class CMP>
// not inline
bool avl_array<T, A, W, P>::binary_search               // Return true iff found
    (typename avl_array<T, A, W, P>::const_reference t, // What to search
     typename avl_array<T, A, W, P>::node_t **pp, // Where it is / should be
