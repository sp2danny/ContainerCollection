...
inline void
avl_array<T, A, W, P>::swap(typename avl_array<T, A, W, P>::iterator it1,
                            typename avl_array<T, A, W, P>::iterator it2) {
  swap_nodes(it1.ptr, it2.ptr);
}

template <class T, class A, class W, class P>
inline void avl_array<T, A, W, P>::swap(
...
template <class T, class A, class W, class P>
inline void
avl_array<T, A, W, P>::move(typename avl_array<T, A, W, P>::iterator it,
...
template <class T, class A, class W, class P>
inline void
avl_array<T, A, W, P>::move(typename avl_array<T, A, W, P>::reverse_iterator it,
                            typename avl_array<T, A, W, P>::difference_type n) {
...
template <class T, class A, class W, class P>
inline void
avl_array<T, A, W, P>::move(typename avl_array<T, A, W, P>::iterator src,
                            typename avl_array<T, A, W, P>::iterator dst) {
  move_node(src.ptr, dst.ptr);
}

template <class T, class A, class W, class P>
inline void avl_array<T, A, W, P>::move(
...
template <class T, class A, class W, class P>
inline void
avl_array<T, A, W, P>::move(typename avl_array<T, A, W, P>::iterator src_from,
...
avl_array<T, A, W, P>::move(typename avl_array<T, A, W, P>::iterator src_from,
  ...
  n = src_to - src_from;
  if (n > 0)
    move_nodes(src_from, size_type(n), dst.ptr);
}
...
inline void avl_array<T, A, W, P>::move(
  ...
  n = src_to - src_from;
  if (n > 0)
    move_nodes(src_from, size_type(n), dst.ptr->m_next, true);
}
...
avl_array<T, A, W, P>::splice(typename avl_array<T, A, W, P>::iterator dst,
  ...
  move_nodes(src.begin(), src.size(), dst.ptr);
}

template <class T, class A, class W, class P>
inline void avl_array<T, A, W, P>::splice(
    typename avl_array<T, A, W, P>::reverse_iterator dst,
    typename avl_array<T, A, W, P>::my_class &src) {
  AA_ASSERT_HO(owner(dst.ptr) == this);
  AA_ASSERT(&src != this);

  move_nodes(src.begin(), src.size(), dst.ptr->m_next, true); // Reverse
}
...
inline void avl_array<T, A, W, P>::splice(
    typename avl_array<T, A, W, P>::iterator dst,
    [[maybe_unused]] typename avl_array<T, A, W, P>::my_class &src,
    typename avl_array<T, A, W, P>::iterator src_from) {
  AA_ASSERT_HO(owner(dst.ptr) == this);
...
inline void avl_array<T, A, W, P>::splice(
  ...
  if (n > 0)
    move_nodes(src_from, n, dst.ptr->m_next, true); // Reverse
}
...
template <class T, class A, class W, class P>
// not inline
void avl_array<T, A, W, P>::swap_nodes(
  ...
  AA_ASSERT_EXC(p->m_parent && q->m_parent,
  ...
  if (p->m_node_width != q->m_node_width) {
    do {
      p->m_total_width = p->left_width() + p->right_width() + p->m_node_width;
      p = p->m_parent;
    }          // If necesary, update
    ...
    do         // to root
    {
      q->m_total_width = q->left_width() + q->right_width() + q->m_node_width;
      q = q->m_parent;
    } while (q);
  }
}
...
template <class T, class A, class W, class P>
// not inline
void avl_array<T, A, W, P>::move_node(
    typename avl_array<T, A, W, P>::node_t *p,
    typename avl_array<T, A, W, P>::difference_type n) {
  int side;
  node_t *q, *r;
  ...
  AA_ASSERT_EXC(p->m_parent, invalid_op_with_end()); // Can't move end()
...
template <class T, class A, class W, class P>
inline void
avl_array<T, A, W, P>::move_node(typename avl_array<T, A, W, P>::node_t *p,
                                 typename avl_array<T, A, W, P>::node_t *q) {
  AA_ASSERT(p); // NULL pointer dereference
  AA_ASSERT(q); // NULL pointer dereference
  ...
  AA_ASSERT_EXC(p->m_parent, invalid_op_with_end()); // Can't move end node
...
template <class IT>
// not inline
void avl_array<T, A, W, P>::move_nodes(
    IT src_from,                                 // Source
    ...
    bool reverse)                                // Dest. direction
{
  ...
  function_requires<Mutable_RandomAccessIteratorConcept<IT>>();
#else
  function_requires<Mutable_BidirectionalIteratorConcept<IT>>();
#endif
#endif
