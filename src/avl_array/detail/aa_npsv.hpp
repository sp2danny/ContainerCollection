...
// not inline
void avl_array<T, A, W, P>::npsv_update_sums() const {
  node_t *p;
...
template <class T, class A, class W, class P>
inline W avl_array<T, A, W, P>::npsv_width() const {
...
template <class T, class A, class W, class P>
inline W avl_array<T, A, W, P>::npsv_width(
...
template <class T, class A, class W, class P>
// not inline
void avl_array<T, A, W, P>::npsv_set_width(
    const typename avl_array<T, A, W, P>::iterator &it, W w, bool update_sums) {
  node_t *p;
...
template <class T, class A, class W, class P>
// not inline
W avl_array<T, A, W, P>::npsv_pos_of(
    typename avl_array<T, A, W, P>::const_iterator it) const {
  W pos;
...
avl_array<T, A, W, P>::npsv_at_pos(W pos) {
  node_t *p;
  W left, right;
  ...
  if (size() == 0 || pos < W(0) ||
      ...
      (pos == node_t::m_total_width && node_t::m_prev->m_node_width != W(0)))
    return dummy();
...
avl_array<T, A, W, P>::npsv_at_pos(W pos) const {
...
// not inline
typename avl_array<T, A, W, P>::iterator
avl_array<T, A, W, P>::npsv_at_pos(W pos, CMP cmp) {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<BinaryFunctionConcept<CMP, int, const W &, const W &>>();
#endif
  ...
  if (size() == 0 || cmp(pos, W(0)) < 0 ||            // pos<(W)0
  ...
  while (p) {
    ...
    right = left + p->m_node_width;

    if ((c = cmp(pos, left)) < 0 ||                // pos < left
        (p->m_children[L] && c == 0 &&             // pos == left
         cmp(p->m_prev->m_node_width, W(0)) == 0)) // prev_w == 0
      p = p->m_children[L];
    else if ((c = cmp(pos, right)) < 0 ||       // pos < right
             (c == 0 &&                         // pos == right
              cmp(p->m_node_width, W(0)) == 0)) // p_w == 0
      return p;
    else {
      pos -= right;
...
template <class T, class A, class W, class P>
template <class CMP>
inline typename avl_array<T, A, W, P>::const_iterator
avl_array<T, A, W, P>::npsv_at_pos(W pos, CMP cmp) const {
#ifdef BOOST_CLASS_REQUIRE
  function_requires<BinaryFunctionConcept<CMP, int, const W &, const W &>>();
#endif

  return (const_cast<my_class *>(this))->npsv_at_pos(pos, cmp);
}
