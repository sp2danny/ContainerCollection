...
bool avl_array<T, A, W, P>::
operator==(const typename avl_array<T, A, W, P>::my_class &a) const {
  if (size() != a.size())
    return false;      // If they have different
                       // sizes, they can't be
...
operator<(const typename avl_array<T, A, W, P>::my_class &a) const {
  if (!size())
    return a.size() != 0; // Both empty --> equal
                          // This empty --> lesser
  const_iterator i, j;
