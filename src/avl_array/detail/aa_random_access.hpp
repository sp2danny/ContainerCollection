...
inline typename avl_array<T, A, W, P>::reference avl_array<T, A, W, P>::
operator[](typename avl_array<T, A, W, P>::size_type n) {
...
template <class T, class A, class W, class P>
inline typename avl_array<T, A, W, P>::reference
avl_array<T, A, W, P>::at(typename avl_array<T, A, W, P>::size_type n) {
  return operator[](n); // And at() too
}

template <class T, class A, class W, class P>
inline typename avl_array<T, A, W, P>::const_reference avl_array<T, A, W, P>::
operator[](typename avl_array<T, A, W, P>::size_type n) const {
...
template <class T, class A, class W, class P>
inline typename avl_array<T, A, W, P>::const_reference
avl_array<T, A, W, P>::at(typename avl_array<T, A, W, P>::size_type n) const {
  return operator[](n);
}
...
avl_array<T, A, W, P>::position_of_node(
    ...
    typename avl_array<T, A, W, P>::my_class *&a, bool reverse) {
  size_type pos;
  const node_t *parent;
...
template <class T, class A, class W, class P>
// not inline
typename avl_array<T, A, W, P>::node_t *avl_array<T, A, W, P>::node_at_pos(
...
avl_array<T, A, W, P>::jump(typename avl_array<T, A, W, P>::node_t *p,
  ...
  if (!n)
    return p; // Stupid
  if (!p->m_parent && p->m_count == 1)
    return NULL; // cases

  if (reverse &&      // Special case: jump _from_ rend
      !p->m_parent) { // Go to the first element, and
