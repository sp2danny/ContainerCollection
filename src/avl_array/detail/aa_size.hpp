...
inline typename avl_array<T, A, W, P>::size_type
...
template <class T, class A, class W, class P>
inline bool avl_array<T, A, W, P>::empty() const {
  return size() == 0;
}
...
template <class T, class A, class W, class P>
// not inline
typename avl_array<T, A, W, P>::size_type avl_array<T, A, W, P>::max_size() {
  // If pointers are smaller or eq.
  // to size_type, the limit is
  // imposed by the address space

  if (sizeof(void *) <= sizeof(size_type))
    return ((size_type(1) << ((sizeof(void *) << 3) - 1)) /
            sizeof(payload_node_t))
           << 1;

  size_type mxp, mxu, r, mn; // Otherwise, it depends on
                             // the sizes ratio
  mxp = ((size_type(1) << ((sizeof(size_type) << 3) - 1)) /
         sizeof(payload_node_t))
        << 1;

  mn = sizeof(difference_type) < sizeof(size_type) ? sizeof(difference_type)
                                                   : sizeof(size_type);
  ...
  return (mxu >> r) < mxp ? // If the index type size is more
             mxu
                          : // restrictive, choose it. Otherwise
...
void avl_array<T, A, W, P>::resize(
  ...
  if (n <= 0)                             // respect to the
    clear();                              // current size, resize
           ...
           (n < sz && worth_rebuild(sz - n, sz, true))) {
...
template <class T, class A, class W, class P>
template <class DP>
void avl_array<T, A, W, P>::resize(typename avl_array<T, A, W, P>::size_type n,
                                   DP &dp) {
  node_t *first, *last;
  node_t *p, *next;
