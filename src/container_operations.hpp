...
auto remove(pick_1, C1& c1, const Itm& itm) -> decltype(c1.remove(itm), void())
{
#ifdef FULL_DIAG
...
template<typename C1, typename Itm>
auto remove(pick_2, C1& c1, const Itm& itm) -> std::enable_if_t<std::is_base_of<
	std::random_access_iterator_tag, typename std::iterator_traits<decltype(std::begin(c1))>::iterator_category>::value>
{
#ifdef FULL_DIAG
...
template<typename C1, typename Itm>
auto binary_find(pick_1, C1&& c1, const Itm& itm) -> std::pair<bool, decltype(c1.binary_find(itm))>
{
...
template<typename C1, typename Itm>
auto binary_find(pick_2, C1&& c1, const Itm& itm) -> std::pair<bool, decltype(c1.lower_bound(itm))>
{
...
template<typename C1, typename Itm>
auto binary_find(pick_3, C1&& c1, const Itm& itm) -> std::pair<bool, decltype(c1.begin())>
{
...
template<typename C1, typename Itm>
auto binary_find(pick_4, const C1& c1, const Itm& itm) -> std::pair<bool, typename C1::const_iterator>
{
	using std::lower_bound;
	auto iter = lower_bound(c1.begin(), c1.end(), itm);
	if (iter == c1.end())
...
auto splice(pick_1, Cont& src, It sb, It se, Cont& dst, It d) -> decltype(dst.splice(d, src, sb, se), void())
{
...
template<typename Cont>
auto merge(pick_2, Cont& c1, Cont& c2) -> void
{
	Cont dst;
...
template<typename Cont>
auto integrity(pick_2, Cont& c1) -> bool
{
...
template<typename Cont>
auto reverse(pick_2, Cont& c1) -> decltype(std::reverse(c1.begin(), c1.end()), void())
{
