...
template <typename T> class splice_list {
...
public:
  ...
  template <typename It> iterator insert(iterator itr, It b, It e) {
    if (b == e)
      return itr;
    while (true) {
      itr = insert(itr, *b);
  ...
  void splice(iterator pos, splice_list &&, iterator it) { splice(pos, it); }
  void splice(iterator pos, splice_list &, iterator first, iterator last) {
  ...
  template <typename Op> void sort(Op op);

  void experimental_merge_sort();

  void merge(splice_list &other) { merge(other, std::less<T>{}); }
  void merge(splice_list &&other) { merge(other); }
  template <typename Op> void merge(splice_list &other, Op op);
  template <typename Op> void merge(splice_list &&other, Op op) {
    merge(other, op);
  }

  void unique() { unique(std::equal_to<T>{}); }
  template <class Eq> void unique(Eq eq);

  void reverse();

  void remove(const T &val);
  template <typename Pred> Pred remove_if(Pred pred);
...
private:
  template <typename Stream> static Stream &helper_print(Stream &, Sentry &);
  ...
  template <typename Op> static void helper_sort(Sentry &, Op);
  ...
  template <typename... Args> NodeP helper_makenode(Args &&...);
...
template <typename T>
splice_list<T>::splice_list(std::initializer_list<T> il)
    : splice_list(il.begin(), il.end()) {}
...
template <typename T> void splice_list<T>::assign(std::initializer_list<T> il) {
  assign(il.begin(), il.end());
}

template <typename T> splice_list<T>::splice_list() {
...
template <typename T> void splice_list<T>::assign(std::size_t n, const T &val) {
  clear();
  helper_assign(n, val);
}

template <typename T> void splice_list<T>::assign(const splice_list &other) {
  assign(other.begin(), other.end());
}

template <typename T> void splice_list<T>::assign(splice_list &&other) {
  swap(other);
}

template <typename T> void splice_list<T>::swap(splice_list &other) noexcept {
  using std::swap;
...
template <typename T> void splice_list<T>::clear() {
  while (!empty())
    pop_back();
}
...
/// </summary>
template <typename T> std::size_t splice_list<T>::size() const noexcept {
...
template <typename T> bool splice_list<T>::empty() const noexcept {
...
template <typename T> void splice_list<T>::push_back(const T &t) {
  NodeP p = helper_makenode(t);
  link(sentinel->prev, p);
  link(p, sentinel);
}

template <typename T> void splice_list<T>::push_back(T &&t) {
  NodeP p = helper_makenode(std::move(t));
  link(sentinel->prev, p);
  link(p, sentinel);
}

template <typename T> void splice_list<T>::push_front(const T &t) {
  NodeP p = helper_makenode(t);
  link(p, sentinel->next);
  link(sentinel, p);
}

template <typename T> void splice_list<T>::push_front(T &&t) {
  NodeP p = helper_makenode(std::move(t));
  link(p, sentinel->next);
  link(sentinel, p);
}
...
template <typename T> void splice_list<T>::pop_back() {
  NodeP p = sentinel->prev;
  link(p->prev, p->next);
  delete p;
}

template <typename T> void splice_list<T>::pop_front() {
  NodeP p = sentinel->next;
  link(p->prev, p->next);
  delete p;
}
...
template <typename T> auto splice_list<T>::erase(iterator what) -> iterator {
  NodeP p = what.node;
  NodeP n = p->next;
  link(p->prev, p->next);
  delete p;
...
template <typename T> void splice_list<T>::link(NodeP n1, NodeP n2) {
  n1->next = n2;
  n2->prev = n1;
}
...
template <typename T> void splice_list<T>::splice(iterator pos, iterator it) {
...
template <typename T> template <typename Op> void splice_list<T>::sort(Op op) {
  helper_sort(*(Sentry *)sentinel, op);
}

template <typename T> void splice_list<T>::experimental_merge_sort() {
...
void splice_list<T>::merge(splice_list &other, Op op) {
  ...
  if (empty())
    return swap(other);
  helper_merge(*(Sentry *)sentinel, *(Sentry *)sentinel,
               *(Sentry *)other.sentinel, op);
  link(other.sentinel, other.sentinel);
}
...
template <typename T> template <class Eq> void splice_list<T>::unique(Eq eq) {
  iterator i = begin();
  if (i == end())
    return;
  iterator curr = i;
...
template <typename T> void splice_list<T>::reverse() {
  if (empty())
    return;
  NodeP p1 = sentinel;
  NodeP p2 = p1->next;
...
template <typename T> void splice_list<T>::remove(const T &val) {
