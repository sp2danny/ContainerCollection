...
struct Plot {
  Plot();
  template <typename It> void AddPoints(It, It);
...
struct MultiPlot {
  MultiPlot();
  template <typename It> void AddPoints(RGB, It, It);
...
template <typename It> void Plot::AddPoints(It b, It e) {
  while (b != e) {
    auto[x, y] = *b;
    AddPoint(x, y);
    ++b;
...
template <typename It> void MultiPlot::AddPoints(RGB pix, It b, It e) {
  while (b != e) {
    auto[x, y] = *b;
    AddPoint(pix, x, y);
    ++b;
