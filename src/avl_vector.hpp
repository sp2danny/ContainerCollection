...
public:
	...
	struct iterator
	{
		...
		{
			return std::ptrdiff_t(avp->internal_indexof(node)) - std::ptrdiff_t(avp->internal_indexof(other.node));
		}
	...
	struct reverse_iterator
	{
		...
		{
			return std::ptrdiff_t(avp->internal_indexof(other.node)) - std::ptrdiff_t(avp->internal_indexof(node));
		}
	...
	void sort(Op op = Op{})
	{
		VNP vnp;
		internal_flatten(vnp);
		auto nless = [&op](NodeP lhs, NodeP rhs) -> bool { return op(lhs->item, rhs->item); };
	...
	void merge(vector& other, Op op = Op{})
	{
		...
		auto cmp = [&op](NodeP lhs, NodeP rhs) -> bool { return op(lhs->item, rhs->item); };
	...
	iterator upper_bound(const T& val) { return {this, internal_sorted_insert_position(val)}; }
	/// lower_bound
	iterator lower_bound(const T& val) { return {this, internal_lower_bound(val)}; }
	/// return first found item == val, or end
	iterator binary_find(const T& val) { return {this, internal_search_node(val)}; }
	bool     binary_search(const T& val) const { return me->internal_search_node(val) != core.root; }
