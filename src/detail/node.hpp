...
{                                 // Note that the dummy has no T
  ...
  protected:
    ...
    long long m_height;   // levels in subtree, including self
    long long m_count;    // nodes in subtree, including self
	...
	W total_width() const { return W(1) + left_width() + right_width(); }

    // Helper functions, all O(1) (no loop, no recursion)

    long long left_count () const;   // Nodes in left subtree
    long long right_count () const;  // Nodes in right subtree

    long long left_height () const;  // Height of left subtree
    long long right_height () const; // Height of right subtree
