...
// iterator from index, including one past (end())
// Complexity: O(log(N))
template<class T,class A,class W,class P>
inline
	typename avl_array<T,A,W,P>::iterator
	avl_array<T,A,W,P>::iat
		(typename avl_array<T,A,W,P>::size_type n)
{
	AA_ASSERT_EXC (n>=0 && n<=size(), index_out_of_bounds());  // Index out of range

	if( n == size() ) return end();

	return iterator{node_at_pos(n)};
}


// Indexation operator: return the n'th element (by
// reference). operator[] is unchecked, but both
// at() and operator() check
// bounds and throw an exception if necessary.
// Const versions are provided too.
//
// Complexity: O(log(N))
  ...
  (typename avl_array<T,A,W,P>::size_type n)
{
  ...
  return operator[](n);     // Operator() does the same as operator[], plus range checking
}
...
inline
  ...
  (typename avl_array<T,A,W,P>::size_type n)
{
  return operator()(n);     // at() does the same as operator(), plus range checking
}
