...
template <typename T = void> struct integrity {
  bool operator()() { return true; }
  template <typename C1, typename... Args> bool operator()(C1 &, Args &...);
  static std::string name() { return "integrity"s; }
};

template <typename T = void> struct copy_to {
  template <typename C1> void operator()(const C1 &) {}
...
template <typename T = void> struct compare {
  template <typename C1> bool operator()(const C1 &) { return true; }
...
template <typename T = void> struct insert_nth {
...
template <typename T = void> struct insert {
  insert(std::size_t count = 1) : count(count) {}
  template <typename C1, typename... Args> void operator()(C1 &, Args &...);
...
template <typename T = void> struct erase_nth {
...
template <typename T = void> struct erase {
...
template <typename T = void> struct sort {
  void operator()() {}
...
template <typename T = void> struct unique {
  void operator()() {}
...
template <typename T = void> struct sort_unique {
  void operator()() {}
...
template <typename T = void> struct print {
...
template <typename T = void> struct remove {
  template <typename Itm> void operator()(const Itm &) {}
...
template <typename T = void> struct splice_merge {
  void operator()() {}
...
template <typename T = void> struct binary_find_swap {
  template <typename Itm> void operator()(const Itm &, const Itm &) {}
...
template <typename T = void> struct nth_swap {
...
template <typename T = void> struct reverse {
  void operator()() {}
  template <typename C1, typename... Args> void operator()(C1 &, Args &...);
};

template <typename T = void> struct clear {
  void operator()() {}
  template <typename C1, typename... Args> void operator()(C1 &, Args &...);
};
  ...
  for (auto &&x : time_data) {
    if (nameof(Excl{}) != x.first) {
      ...
      if (sz) {
        ...
        std::cout << "Container totals : " << sum * multiplyer / 1000.0 << " "
                  << unit << "\n\n";
      }
    }
  }
...
template <typename T>
template <typename C1, typename C2, typename... Args>
bool CT::compare<T>::operator()(const C1 &orig, const C2 &first,
                                const Args &... rest) {
  init();
  start_clock();
  auto i1 = orig.begin();
...
template <typename T>
template <typename C1, typename... Args>
void CT::insert_nth<T>::operator()(std::size_t idx, int val, C1 &first,
                                   Args &... rest) {
  init();
#ifndef NDEBUG
// std::cout << name() << " of " << nameof(first) << std::endl;
...
#ifdef FULL_DIAG
  std::cerr << "attempting: "
            << "insert_nth of " << val << " into " << nameof(first)
            << " (sz:" << first.size() << ")"
            << " at pos " << idx << clr;
#endif
...
void CT::insert<T>::operator()(C1 &first, Args &... rest) {
  ...
  for (std::size_t i = 0; i < count; ++i) {
    ...
    std::size_t idx =
        std::uniform_int_distribution<std::size_t>{0, sz}(generator);
    auto num = distribution(generator);
...
void CT::erase<T>::operator()(C1 &first, Args &... rest) {
  ...
  for (auto i = 0ul; i < count; ++i) {
    ...
    std::size_t idx =
        std::uniform_int_distribution<std::size_t>{0, sz - 1}(generator);
...
#ifdef FULL_DIAG
  std::cerr << "attempting: "
            << "sort_unique of " << nameof(first) << " (sz:" << first.size()
            << ")" << clr;
#endif
...
#ifdef FULL_DIAG
  std::cerr << "attempting: "
            << "sort of " << nameof(first) << " (sz:" << first.size() << ")"
            << clr;
#endif
...
#ifdef FULL_DIAG
  std::cerr << "attempting: "
            << "unique of " << nameof(first) << " (sz:" << first.size() << ")"
            << clr;
#endif
  start_clock();
  CO::unique(first);
...
template <typename T>
template <typename Itm, typename C1, typename... Args>
void CT::binary_find_swap<T>::operator()(const Itm &itm1, const Itm &itm2,
                                         C1 &first, Args &... rest) {
  start_clock();
...
template <typename T>
template <typename C1, typename... Args>
void CT::nth_swap<T>::swp(std::size_t idx1, std::size_t idx2, C1 &first,
                          Args &... rest) {
  start_clock();
  auto itr1 = nth(first, idx1);
  auto itr2 = nth(first, idx2);
