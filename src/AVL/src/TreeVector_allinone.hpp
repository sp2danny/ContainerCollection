...
auto helper_compare(pick_1, const V1& v1, const V2& v2) -> decltype((int)v1.compare(v2))
{
	return v1.compare(v2);
}

template<typename V1, typename V2>
auto helper_compare(pick_2, const V1& v1, const V2& v2) -> decltype((int)compare(v1, v2))
{
	return compare(v1, v2);
}

template<typename V1, typename V2>
auto helper_compare(pick_3, const V1& v1, const V2& v2) -> decltype((int)(v1 - v2))
{
	return v1 - v2;
}

template<typename V1, typename V2>
auto helper_compare(pick_4, const V1& v1, const V2& v2) -> decltype(bool(v2 < v1), bool(v1 < v2), int{})
{
	if (v1 < v2)
		return -1;
	if (v2 < v1)
...
template<typename V1, typename V2>
auto helper_compare(pick_5, const V1& v1, const V2& v2) -> decltype(std::less<V1>()(v1, v2), int{})
{
...
auto compare(const C1& c1, const C2& c2) ->
    typename std::enable_if<std::is_same<decltype(std::begin(c1)), decltype(std::end(c1))>::value &&
                            std::is_same<decltype(std::begin(c2)), decltype(std::end(c2))>::value &&
                            std::is_same<decltype(*std::begin(c1)), decltype(*std::begin(c2))>::value,
                            int>::type
{
...
auto d_sort(C1& c1, pick_1) -> decltype(c1.stable_sort())
{
	return c1.stable_sort();
}

template<typename C1>
auto d_sort(C1& c1, pick_2) -> decltype(c1.sort())
{
	return c1.sort();
}

template<typename C1>
void d_sort(C1& c1, pick_3)
{
	std::stable_sort(std::begin(c1), std::end(c1));
}
...
struct less<TT>
{
	void* operator()(TT, TT) { return nullptr; }
};
}

 // char operator<(TT,TT) { return false; }
 // long long operator-(TT,TT) { return 0; }
 // short compare(TT,TT) { return 0; }

void foo()
{
	using namespace std;

	// typedef TreeVector<int> TVI;

	const int SZ = 100'000;

	auto t1a = pt_insert_erase<vector<int>>(SZ, 1'000);
	auto t2a = pt_insert_erase<list<int>>(SZ, 1'000);
	auto t3a = pt_insert_erase<TreeVector<int>>(SZ, 1'000);

	cout << "insert / erase : sz " << SZ << endl;
	cout << "\tstd::vector  " << t1a << endl;
	cout << "\tstd::list    " << t2a << endl;
	cout << "\tTreeVector   " << t3a << endl;

	auto t1b = pt_random_access<vector<int>>(SZ, 1'000);
	auto t2b = pt_random_access<list<int>>(SZ, 1'000);
	auto t3b = pt_random_access<TreeVector<int>>(SZ, 1'000);

	cout << "random access : sz " << SZ << endl;
	cout << "\tstd::vector  " << t1b << endl;
	cout << "\tstd::list    " << t2b << endl;
	cout << "\tTreeVector   " << t3b << endl;

	cout << "summation : sz " << SZ << endl;
	cout << "\tstd::vector  " << t1a + t1b << endl;
	cout << "\tstd::list    " << t2a + t2b << endl;
	cout << "\tTreeVector   " << t3a + t3b << endl;

	auto t1c = pt_sort<vector<int>>(SZ, 1);
	auto t2c = pt_sort<list<int>>(SZ, 1);
	auto t3c = pt_sort<TreeVector<int>>(SZ, 1);

	cout << "sort : sz " << SZ << endl;
	cout << "\tstd::vector  " << t1c << endl;
	cout << "\tstd::list    " << t2c << endl;
	cout << "\tTreeVector   " << t3c << endl;
}
