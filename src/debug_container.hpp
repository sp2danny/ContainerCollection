...
private:
	...
	private:
		...
		bool valid;
		core_iterator(debug_container* owner, typename item_list::iterator iter, bool ok=true) : cb(owner->impl), iter(iter), valid(ok) {}
	friend
		class debug_container;
	};
...
public:
	...
	struct const_iterator : std::iterator<std::random_access_iterator_tag, const T>, core_iterator
	{
		...
		const_iterator operator++(int) { auto tmp = *this; next(); return tmp; }
		const_iterator operator--(int) { auto tmp = *this; prev(); return tmp; }
