...
template <typename T> struct wierd_alloc {

  template <class U> wierd_alloc(const wierd_alloc<U> &) {}
  wierd_alloc() = default;

  template <typename U> struct rebind { typedef wierd_alloc<U> other; };
...
private:
  ...
  private:
    ...
    bool valid;
    core_iterator(debug_container *owner, typename item_list::iterator iter,
                  bool ok = true)
  ...
  template <typename... Args> void ci_insert(core_iterator &, Args &&...);
...
public:
  ...
  private:
    iterator(debug_container *owner, typename item_list::iterator iter)
        : core_iterator(owner, iter) {}
  ...
  private:
    const_iterator(debug_container *owner, typename item_list::iterator iter)
        : core_iterator(owner, iter) {}
    friend class debug_container;
  };
  ...
  template <typename... Args> iterator emplace(iterator, Args &&...);
...
debug_container<T, A>::core_iterator::core_iterator()
    : cb(nullptr), valid(false) {}

template <typename T, typename A>
debug_container<T, A>::core_iterator::core_iterator(
    const core_iterator &other) {
  valid = other.valid;
  cb = other.cb;
  iter = other.iter;
  if (cb)
...
template <typename T, typename A>
auto debug_container<T, A>::core_iterator::operator=(const core_iterator &other)
    -> core_iterator & {
  if (cb)
    cb->remi(this);
  valid = other.valid;
...
template <typename T, typename A>
auto debug_container<T, A>::core_iterator::operator=(core_iterator &&other)
    -> core_iterator & {
  auto old_cb = cb;
  if (cb)
    cb->remi(this);
...
template <typename T, typename A> void debug_container<T, A>::clear() {
...
template <typename T, typename A> bool debug_container<T, A>::empty() const {
  assert(impl);
...
template <typename T, typename A> debug_container<T, A>::debug_container() {
  impl = new_block();
}
...
template <typename T, typename A>
debug_container<T, A>::debug_container(debug_container &&other)
    : debug_container() {
  swap(other);
}
...
template <typename T, typename A>
auto debug_container<T, A>::operator=(const debug_container &)
    -> debug_container & {
  return *this;
}

template <typename T, typename A>
auto debug_container<T, A>::operator=(debug_container &&other) noexcept
    -> debug_container & {
  swap(other);
  return *this;
}
