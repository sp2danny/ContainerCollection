...
struct InsOp {
  template <typename T> void Execute(std::vector<int> &param, T &cont) {
    int val = param[0];
    int pos = param[1];
...
int randn(int n) { return (int)rand() % n; }
...
struct InsROp {
  ...
  void Print(std::vector<int> &param, std::ostream &out) {
    out << "Insert " << param.size() - 1 << " items at " << param[0]
        << std::endl;
  }
};
...
struct SortOp {
  template <typename T> void Execute(std::vector<int> &, T &cont) {
    CO::sort(cont);
  };
...
template <typename T> void Op::Execute(T &cont) {
  // std::cout << op_num << std::endl;
  // for (auto&& x : op_param) std::cout << x << " ";
  // std::cout << std::endl;
  switch (op_num) {
  case InsOpIdx:
    InsOp{}.Execute(op_param, cont);
    break;
  case DelOpIdx:
    DelOp{}.Execute(op_param, cont);
    break;
  case InsROpIdx:
    InsROp{}.Execute(op_param, cont);
    break;
  case DelROpIdx:
    DelROp{}.Execute(op_param, cont);
    break;
  case SortOpIdx:
    SortOp{}.Execute(op_param, cont);
    break;
  case ReverseOpIdx:
    ReverseOp{}.Execute(op_param, cont);
    break;
  }
}
...
void Op::Print(std::ostream &out) {
  switch (op_num) {
  case InsOpIdx:
    InsOp{}.Print(op_param, out);
    break;
  case DelOpIdx:
    DelOp{}.Print(op_param, out);
    break;
  case InsROpIdx:
    InsROp{}.Print(op_param, out);
    break;
  case DelROpIdx:
    DelROp{}.Print(op_param, out);
    break;
  case SortOpIdx:
    SortOp{}.Print(op_param, out);
    break;
  case ReverseOpIdx:
    ReverseOp{}.Print(op_param, out);
    break;
  }
}
