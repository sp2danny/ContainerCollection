...
template <typename T, std::size_t N> class inline_vector {
...
public:
  ...
  template <typename It> inline_vector(It, It);
  ...
  template <typename It> iterator insert(iterator, It, It);
...
inline_vector<T, N>::inline_vector(const inline_vector &other) {
  ...
  T *dst = data();
  if
    constexpr(triv) { std::memcpy(dst, src, sizeof(T) * ic.size); }
  else {
...
inline_vector<T, N>::inline_vector(inline_vector &&other) noexcept(cne ||
  ...
  if (other.ic.size <= N) {
    ...
    T *dst = data();
    if
      constexpr(triv) { memcpy(dst, src, sizeof(T) * ic.size); }
    else {
      for (size_type i = 0; i < ic.size; ++i) {
...
template <typename T, std::size_t N>
template <typename It>
inline_vector<T, N>::inline_vector(It b, It e) : inline_vector() {
...
template <typename T, std::size_t N> void inline_vector<T, N>::make_heap() {
  if (ic.capa)
    return;
  auto sz = ic.size;
  auto cp = sz + 1;
  ...
  T *src = data();
  if
    constexpr(triv) { std::memcpy(ptr, src, sizeof(T) * sz); }
  else {
...
// destruction
template <typename T, std::size_t N> void inline_vector<T, N>::clear() {
  T *p = data();
  size_type i, n = size();
  for (i = 0; i < n; ++i) {
    (p + i)->~T();
...
template <typename T, std::size_t N> inline_vector<T, N>::~inline_vector() {
  clear();
}

// assignment
template <typename T, std::size_t N>
auto inline_vector<T, N>::operator=(const inline_vector &other)
    -> inline_vector & {
  clear();
  if (other.ic.size <= N) {
    ic.size = other.ic.size;
...
void inline_vector<T, N>::reserve(size_type sz) {
  ...
  auto n = size();
  if
    constexpr(triv) { std::memcpy(ptr, src, n * sizeof(T)); }
  else {
...
template <typename T, std::size_t N> bool inline_vector<T, N>::empty() const {
  return size() == 0;
}
...
template <typename T, std::size_t N> void inline_vector<T, N>::shrink_to_fit() {
  if (ic.capa == 0)
    return;
  auto sz = hc.size;
  if (hc.capa == sz)
  ...
  if
    constexpr(triv) { memcpy(dst, src, sizeof(T) * sz); }
  else {
...
template <typename T, std::size_t N> T *inline_vector<T, N>::data() {
  if (ic.capa == 0)
    return ic.data;
  else
    return hc.data;
