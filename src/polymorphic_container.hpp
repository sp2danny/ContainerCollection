...
template <typename, typename, typename> struct iterator;

template <typename T, typename underlying_iterator> struct iterator_base {
...
private:
  ...
  template <typename, typename, typename> friend struct iterator;
};

template <typename T, typename underlying_iterator, typename category>
struct iterator : std::iterator<category, T>,
                  iterator_base<T, underlying_iterator> {
  iterator() = default;
  ...
  operator iterator<T2, UI2, C2>() {
    return {this->iter};
  }

  template <typename U, typename UI2, typename C2> friend struct iterator;
};
...
template <typename T, template <typename...> class Underlying = std::vector,
...
private:
  ...
  template <typename U> deleter_t make_deleter();

  template <typename U> static Allocator<U> &allocator() {
  ...
  typedef
      typename underlying_container::const_iterator underlying_const_iterator;
  typedef typename std::iterator_traits<underlying_iterator>::iterator_category
      underlying_iterator_category;

  template <typename U> struct sub_or_same {
    using CT = detail::clean<T>;
    using CU = detail::clean<U>;
    static const bool value =
        std::is_base_of<CT, CU>::value || std::is_same<CT, CU>::value;
  };

  static const bool underlying_swap_noexcept =
      noexcept(std::declval<underlying_container &>().swap(
          std::declval<underlying_container &>()));
...
public:
  ...
  void
  assign(polymorphic_container &&other) noexcept(underlying_swap_noexcept) {
    data.swap(other.data);
  }

  void clear() { data.clear(); }
  void swap(polymorphic_container &other) noexcept(underlying_swap_noexcept) {
    data.swap(other.data);
  }
  void swap(polymorphic_container &&other) noexcept(underlying_swap_noexcept) {
    data.swap(other.data);
  ...
  typedef detail::iterator<T, underlying_iterator, underlying_iterator_category>
      iterator;
  typedef detail::iterator<const T, underlying_const_iterator,
                           underlying_iterator_category>
      const_iterator;
...
private:
  template <typename U> iterator explicit_insert(const_iterator, const U &);

  template <typename U> iterator explicit_insert(const_iterator, U &&);
...
public:
  ...
  iterator insert(const_iterator,
                  T *); // takes ownership, uses delete for disposal

  template <typename Deleter>
  iterator insert(const_iterator, T *,
                  const Deleter &); // takes ownership, uses custom deleter

  template <typename U = T, typename... Args,
            typename = std::enable_if_t<sub_or_same<U>::value>>
  ...
  T &operator[](std::size_t idx) {
    static_assert(detail::sub_or_same<underlying_iterator_category,
...
void swap(polymorphic_container<T, U, A> &lhs,
          polymorphic_container<T, U, A> &rhs) {
  lhs.swap(rhs);
}

template <typename T, template <typename...> class U,
          template <typename...> class A>
int polymorphic_container<T, Underlying, Allocator>::compare(
    const polymorphic_container &rhs) const {
  auto li = lhs.begin();
  auto ri = rhs.begin();
  auto &&lhs = *this;
  while (true) {
...
template <typename T, template <typename...> class U,
          template <typename...> class A>
bool operator==(polymorphic_container<T, U, A> &lhs,
                polymorphic_container<T, U, A> &rhs) {
...
void polymorphic_container<T, Underlying, Allocator>::Item::swap(
    Item &other) noexcept {
  using std::swap;
...
template <typename T, template <typename...> class Underlying,
          template <typename...> class Allocator>
template <typename U>
auto polymorphic_container<T, Underlying, Allocator>::make_deleter()
    -> deleter_t {
...
template <typename T, template <typename...> class Underlying,
          template <typename...> class Allocator>
auto polymorphic_container<T, Underlying, Allocator>::Item::operator*() const
    -> T & {
  assert(value);
  return *value;
}
...
template <typename T, template <typename...> class Underlying,
          template <typename...> class Allocator>
template <typename U>
auto polymorphic_container<T, Underlying, Allocator>::explicit_insert(
    const_iterator i, const U &u) -> iterator {
  assert((typeid(u) == typeid(U)) &&
         "allocator::deallocate need to know the exact type");
...
template <typename T, template <typename...> class Underlying,
          template <typename...> class Allocator>
auto polymorphic_container<T, Underlying, Allocator>::insert(const_iterator i,
                                                             T *t) -> iterator {
  auto iter = data.emplace(i.iter);
  iter->value = t;
...
auto polymorphic_container<T, Underlying, Allocator>::emplace(const_iterator i,
                                                              Args &&... args)
    -> iterator {
  U *space = allocator<U>().allocate(1);
...
template <typename T, int Blocksize> struct MyAlloc {
  typedef T value_type;
  typedef T *pointer;
  ...
  MyAlloc() {
    ...
    if (first) {
      std::cout << "allocator instatiated on a " << typeid(T).name()
                << std::endl;
      first = false;
    } else {
      std::cout << "caller assumes stateless, " << typeid(T).name()
                << std::endl;
    }
  }
  ...
  template <class U>
  MyAlloc(const MyAlloc<U, Blocksize> &) noexcept : MyAlloc() {}

  template <typename U> struct rebind { typedef MyAlloc<U, Blocksize> other; };
...
template <typename T> void printit(const T &cont) {
  for (auto &&x : cont) {
    std::cout << x << "  ";
  }
  std::cout << std::endl;
...
template <typename T> using Alloc = MyAlloc<T, 1024>;
